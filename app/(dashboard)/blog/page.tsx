...
import { MoreHorizontal, Plus, Pencil, Trash2, BookOpen, CheckCircle, XCircle } from 'lucide-react';
...
export default function BlogPage() {
  ...
  const handleApprove = (blog: Blog) => updateBlog.mutate({ id: blog.id, isActive: true });

  const handleReject = (blog: Blog) => updateBlog.mutate({ id: blog.id, isActive: false });
  ...
  const columns: Column<Blog>[] = useMemo(
    () => [
        ...
        cell: (blog) => (
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              by{' '}
              {blog.user?.id != null ? (
                <Link href={`/users/${blog.user.id}`} className="text-blue-600 hover:underline">
      ...
      },
      {
        key: 'category',
        header: 'Category',
        cell: (blog) => <span className="text-sm">{blog.category?.name ?? '—'}</span>,
      },
      {
        key: 'isActive',
        header: 'Status',
      ...
      },
      {
        ...
        header: 'Created',
        cell: (blog) => (
          <span className="text-sm text-muted-foreground">{formatDate(blog.createdAt)}</span>
        ),
      },
      {
        key: 'actions',
