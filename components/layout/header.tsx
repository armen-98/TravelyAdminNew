...
export function Header() {
  ...
  useEffect(() => {
    ...
    setCanUseLocalEnvironment(host === 'localhost' || host === '127.0.0.1' || host === '::1');
  }, []);
  ...
  const handleEnvironmentChange = async (value: string) => {
    ...
    try {
      ...
      const { data } = await axios.post<{
        user: { id: number; fullName: string; email: string; role: { id: number; name: string } };
        token: string;
      }>(`${newBaseUrl}/auth/token-by-email`, { email });
        ...
        {isAdmin ? (
          ...
          >
            ...
            <SelectContent>
              ...
              {canUseLocalEnvironment ? <SelectItem value="local">Local</SelectItem> : null}
            </SelectContent>
          </Select>
        ) : null}
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-9 w-9">
                <AvatarImage src={me?.profileImage?.url ?? ''} alt={displayName ?? ''} />
