        ...
        <Card>
            ...
            <CardContent className="space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-4">
                    {data.checkInTime && (
                        <div><p className="text-xs text-muted-foreground">Check-in</p><p>{data.checkInTime}</p></div>
                    )}
                    {data.checkOutTime && (
                        <div><p className="text-xs text-muted-foreground">Check-out</p><p>{data.checkOutTime}</p></div>
                    )}
                    {data.petsAllowed != null && (
                        <div><p className="text-xs text-muted-foreground">Pets</p>
                            <p>{data.petsAllowed ? "Allowed" : "Not allowed"}</p></div>
                    )}
                    {data.ageRestriction && (
                        <div><p className="text-xs text-muted-foreground">Age restriction</p>
                            <p>{data.ageRestriction}</p></div>
                    )}
                </div>
...
function TransportDetails({data}: { data: Transport }) {
    return (
        <Card>
            ...
            <CardContent className="space-y-4 text-sm">
                {data.operator && (
                    <div><p className="text-xs text-muted-foreground">Operator</p><p>{data.operator}</p></div>
                )}
                ...
                {data.rentalOptions && Object.keys(data.rentalOptions).length > 0 && (
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {data.rentalOptions.perHour != null && <div><span className="text-muted-foreground">Per hour: </span>${data.rentalOptions.perHour}
                            </div>}
                            {data.rentalOptions.perDay != null && <div><span
                                className="text-muted-foreground">Per day: </span>${data.rentalOptions.perDay}</div>}
                            {data.rentalOptions.perWeek != null && <div><span className="text-muted-foreground">Per week: </span>${data.rentalOptions.perWeek}
                            </div>}
                            {data.rentalOptions.perMonth != null && <div><span className="text-muted-foreground">Per month: </span>${data.rentalOptions.perMonth}
                            </div>}
                        </div>
                    </div>
                )}
                            ...
                            {data.practitioners.map((p, i) => (
                                <div key={i} className="rounded-md border px-3 py-2">
                                    ...
                                    {p.yearsOfExperience != null &&
                                        <p className="text-xs text-muted-foreground">{p.yearsOfExperience} years
                                            experience</p>}
                                </div>
                            ))}
                        </div>
...
function NatureOutdoorsDetails({data}: { data: NatureOutdoors }) {
    return (
        <Card>
            ...
            <CardContent className="space-y-4 text-sm">
                {data.entryFee && (
                    <div><p className="text-xs text-muted-foreground">Entry fee</p><p>{data.entryFee}</p></div>
                )}
                {data.bestTimeToVisit && (
                    <div><p className="text-xs text-muted-foreground">Best time to visit</p>
                        <p>{data.bestTimeToVisit}</p></div>
                )}
...
function EntertainmentDetails({data}: { data: Entertainment }) {
    return (
        <Card>
            ...
            <CardContent className="space-y-4 text-sm">
                {data.ageRestriction && (
                    <div><p className="text-xs text-muted-foreground">Age restriction</p><p>{data.ageRestriction}</p>
                    </div>
                )}
                {data.eventSchedule && (
                    <div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-1">
                            {Object.entries(data.ticketPrice).map(([k, v]) => (
                                <div key={k}><span className="capitalize text-muted-foreground">{k}: </span>${String(v)}
                                </div>
                            ))}
                        </div>
                    </div>
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl p-0 overflow-hidden bg-black border-0">
                ...
                <div className="relative flex items-center justify-center min-h-[60vh]">
                    ...
                    <div
                        className="absolute bottom-3 left-1/2 -translate-x-1/2 text-white/70 text-xs bg-black/40 px-2 py-1 rounded-full">
...
export default function PlaceDetailPage() {
    ...
    const categoryDetail = place
        ? place.restaurant ?? place.accommodation ?? place.shopping ?? place.transport ?? place.healthWellness ?? place.natureOutdoors ?? place.entertainment
        : null;

    const hasInfo = place && (
        place.address || place.phone || place.email || place.website ||
        place.description || place.openingHours || place.social ||
        place.priceType || (place.facilities && place.facilities.length > 0) ||
                ...
                <Card className="sm:col-span-2">
                    {place.user?.id ? (
                        <Link href={`/users/${place.user.id}`} className="block h-full">
                            <CardContent
                                className="p-4 flex items-center gap-3 h-full hover:bg-muted/40 transition-colors">
                                {place.user.profileImage?.url ? (
                                    ...
                                    <div
                                        className="h-11 w-11 rounded-full bg-muted flex items-center justify-center text-sm font-semibold text-muted-foreground">
                                        {(place.user.fullName || place.user.email || "U").charAt(0).toUpperCase()}
                                    </div>
                                )}
                                <div className="min-w-0">
                                    <p className="text-xs text-muted-foreground">Creator</p>
                                    <p className="font-semibold truncate">{place.user.fullName || `User #${place.user.id}`}</p>
                                    <p className="text-sm text-muted-foreground truncate">{place.user.email || "—"}</p>
                                </div>
                            </CardContent>
                        </Link>
                    ) : (
                    ...
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-semibold">
                                {place.averageRating != null ? Number(place.averageRating).toFixed(1) : "—"} ({place.reviewCount})
                            </p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <TabsContent value="reviews" className="mt-4">
                    <div className="space-y-3">
                        {reviewsLoading ? (
                            ...
                            <Card>
                                <CardContent className="p-8 text-center text-muted-foreground">No reviews
                                    yet.</CardContent>
                            </Card>
                        )}
                    </div>
                                ...
                                <ul className="space-y-4 border-l-2 border-muted ml-1.5 pl-5">
                                    {moderationEntriesNewestFirst.map((entry: PlaceModerationHistoryItem) => (
                                        <li key={entry.id} className="text-sm relative">
                                            <span
                                                className="absolute -left-[1.15rem] top-1.5 h-2 w-2 rounded-full bg-muted-foreground/40"/>
                                            ...
                                            {entry.eventType === "resubmitted" && entry.reason && (
                                                <div
                                                    className="mt-2 rounded-md border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs">
                                                    <p className="font-medium text-amber-900 dark:text-amber-200">Admin
                                                        feedback at time of re-submit</p>
                                                    <p className="text-muted-foreground mt-1 whitespace-pre-wrap">{entry.reason}</p>
                                                </div>
                                            )}
                                        </li>
                                    ))}
                          ...
                          place.country?.name,
                            ...
                            {(place.latitude != null && place.longitude != null) && (
                                <div className="text-xs text-muted-foreground">
                                    Coordinates: {Number(place.latitude).toFixed(6)}, {Number(place.longitude).toFixed(6)}
                                </div>
                            )}
